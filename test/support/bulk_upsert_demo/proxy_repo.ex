defmodule BulkUpsertDemo.ProxyRepo do
  @moduledoc """
  An `:insert_all_function_module` that records each call by messaging the calling process, then
  delegates to the real repo.

  `BulkUpsert.bulk_upsert/4` runs its transaction in the calling process, so a test that passes
  ...
  def insert_all(schema_or_source, entries, opts) do
    ...
    BulkUpsertDemo.Repo.insert_all(schema_or_source, entries, opts)
  end
end
